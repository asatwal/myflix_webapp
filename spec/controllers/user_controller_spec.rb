  ...
  describe "POST create" do
    context "with valid inputs" do
      ...
      it "creates user" do
        ...
        post :create, user: {email_address: user.email_address, password: user.password, password_confirmation: user.password, full_name: user.full_name}
        expect(assigns(:user)).to eq(User.find_by(email_address: user.email_address))
      end
      ...
      it "sets user_id in session" do
        user = Fabricate.build(:user)
        post :create, user: {email_address: user.email_address, password: user.password, password_confirmation: user.password, full_name: user.full_name}
        expect(session[:user_id]).to eq(User.find_by(email_address: user.email_address).id) 
      end
    end
